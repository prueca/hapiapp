<script lang="ts">
    import _ from 'lodash'
    import accountTypes, { getDisplayText } from '$lib/config/account.types'
    import update from '../update.context.svelte'
    import Icon from '@iconify/svelte'
</script>

<div class="modal" class:modal-open={update.open}>
    <div class="modal-box">
        <div class="heading">
            <p class="mb-4 text-xl font-medium">Update an account</p>
        </div>
        <div>
            <!-- account type -->
            <div class="fieldset" class:error={update.issues.type !== null}>
                <label for="" class="label">Select account type</label>
                <select class="select" bind:value={update.data.type}>
                    <option disabled selected value="">Choose one</option>
                    <option value={accountTypes.DEALER}>
                        {getDisplayText(accountTypes.DEALER)}
                    </option>
                    <option value={accountTypes.HAPISTORE}>
                        {getDisplayText(accountTypes.HAPISTORE)}
                    </option>
                    <option value={accountTypes.DIRECT_STORE}>
                        {getDisplayText(accountTypes.DIRECT_STORE)}
                    </option>
                </select>
                <div class="help-text">
                    {update.issues.type}
                </div>
            </div>

            <!-- name -->
            <fieldset class="fieldset" class:error={update.issues.name !== null}>
                <label for="" class="label">Enter account name</label>
                <input
                    type="text"
                    class="input w-full"
                    placeholder="Account name"
                    bind:value={update.data.name}
                />
                <div class="help-text">
                    {update.issues.name}
                </div>
            </fieldset>

            <!-- address -->
            <fieldset class="fieldset" class:error={update.issues.address !== null}>
                <label for="" class="label">Enter account address</label>
                <input
                    type="text"
                    class="input w-full"
                    placeholder="Complete address"
                    bind:value={update.data.address}
                />
                <div class="help-text">
                    {update.issues.address}
                </div>
            </fieldset>

            <!-- phone -->
            <fieldset class="fieldset" class:error={update.issues.phone !== null}>
                <label for="" class="label">Enter phone number</label>
                <input
                    type="text"
                    class="input w-full"
                    placeholder="Contact number"
                    bind:value={update.data.phone}
                />
                <div class="help-text">
                    {update.issues.phone}
                </div>
            </fieldset>

            <!-- isr code -->
            <fieldset class="fieldset" class:error={update.issues.isrCode !== null}>
                <label for="" class="label">Enter ISR Code</label>
                <input
                    type="text"
                    class="input w-full"
                    placeholder="ISR code"
                    bind:value={update.data.isrCode}
                />
                <div class="help-text">
                    {update.issues.isrCode}
                </div>
            </fieldset>

            <!-- sap code -->
            <fieldset class="fieldset" class:error={update.issues.sapCode !== null}>
                <label for="" class="label">Enter SAP Code</label>
                <input
                    type="text"
                    class="input w-full"
                    placeholder="SAP code"
                    bind:value={update.data.sapCode}
                />
                <div class="help-text">
                    {update.issues.sapCode}
                </div>
            </fieldset>
        </div>
        <div class="modal-action">
            <button class="submit-btn btn" onclick={() => update.submit()}> Update </button>
            <button class="close-btn btn" onclick={() => update.setOpen(false)}> Close </button>
        </div>
    </div>
</div>

<div class="modal" class:modal-open={update.loading}>
    <div class="modal-box">
        <div class="heading">
            <div class="flex items-start gap-2">
                <span class="loading mt-1 loading-lg loading-spinner"></span>
                <div>
                    <p class="mb-1 text-lg font-medium">Updating account...</p>
                    <div class="text-sm">Please wait while we update your account.</div>
                </div>
            </div>
        </div>
    </div>
</div>

<div class="modal" class:modal-open={update.error}>
    <div class="modal-box">
        <div class="heading">
            <div class="flex items-start gap-2">
                <Icon icon="boxicons:message-circle-exclamation" class="-mt-0.5" width="32" />
                <div>
                    <p class="mb-1 font-medium">Account update failed</p>
                    <!-- <p class="mb-1 text-sm">{create.error?.message}</p> -->
                    <p class="mb-1 text-sm">Error: {update.error?.code}</p>
                </div>
            </div>
        </div>
        <div class="modal-action">
            <button class="close-btn btn" onclick={() => update.setOpen(false)}> Close </button>
        </div>
    </div>
</div>

<div class="modal" class:modal-open={update.success}>
    <div class="modal-box">
        <div class="heading">
            <div class="flex items-start gap-2">
                <Icon icon="boxicons:message-circle-check" class="-mt-0.5" width="32" />
                <div>
                    <p class="mb-1 font-medium">Updated successfully</p>
                    <p class="mb-1 text-sm">Your account has been updated</p>
                </div>
            </div>
        </div>
        <div class="modal-action">
            <button class="close-btn btn" onclick={() => update.setOpen(false)}> Close </button>
        </div>
    </div>
</div>

<style lang="postcss">
    @reference 'tailwindcss';

    .fieldset {
        @apply not-last:mb-4;

        select,
        input[type='text'] {
            @apply w-full rounded-lg;
        }

        .help-text {
            @apply hidden text-xs text-(--c1);
        }

        &.error {
            select,
            input[type='text'] {
                @apply border-(--c1);
            }

            .help-text {
                @apply block;
            }
        }
    }

    .close-btn {
        @apply cursor-pointer rounded-lg border-none;
    }
    .submit-btn {
        @apply cursor-pointer rounded-lg border-none bg-(--c1) text-white;
    }
</style>
