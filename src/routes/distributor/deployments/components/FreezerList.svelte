<script lang="ts">
    ...
    import type { DeploymentGroup, GroupedFreezer } from '$lib/types/deployment'
    import deployments from '../deployments.context.svelte'

    let { group }: { group: DeploymentGroup } = $props()

    const modelOf = (f: GroupedFreezer) => `${_.startCase(f.brand)} - ${_.startCase(f.model)}`
</script>

<div class="list">
     {#each group.freezers as freezer (freezer.deploymentId)}
         <div class="freezer-item">
             <div class="freezer-actions">
                 <span class="status">{_.startCase(freezer.status)}</span>

                 <button
                    type="button"
                    class="remove"
                    aria-label={`Remove ${freezer.barcode} from this deployment`}
                    disabled={deployments.isRemoving(freezer.deploymentId)}
                    onclick={() => deployments.removeFreezer(freezer)}
                 >
                     {#if deployments.isRemoving(freezer.deploymentId)}
...
<style lang="postcss">
     ...
     .freezer-actions {
         @apply absolute right-2 top-2 flex items-center gap-1.5;
     }
